...
#################################Lectura de bases Internet fijo#######################################
pathFijo='https://raw.githubusercontent.com/postdatacrc/Mediciones_QoE/main/2023/Fijo/Completo/'
periodo_tope='2023-12-01'

####Primera sección - Información histórica Colombia
def Seccion1Fijo():
    Colombia1Fijo=pd.read_csv(pathFijo+"Colombia-histcomp_month_2018-2023(Diciembre).csv", delimiter=';')
    ...
    Colombia1Fijo['Aggregate Date'] = pd.to_datetime(Colombia1Fijo['Aggregate Date'].replace(diction1Fijo),format='%d-%b-%y',dayfirst=True)
    Colombia1Fijo['month'] = Colombia1Fijo['Aggregate Date'].dt.month
    Colombia1Fijo['year'] = Colombia1Fijo['Aggregate Date'].dt.year
    Colombia1Fijo.drop(['Device','Location', 'Platform', 'Technology Type', 'Metric Type', 'Provider'], axis=1, inplace=True)
    return Colombia1Fijo
Colombia1Fijo=Seccion1Fijo()
Colombia1Fijo=Colombia1Fijo.rename(columns={'Minimum Latency':'Latency'})

####Segunda sección - Información departamental por operadores
def Seccion2Fijo():
    df2_1Fijo=pd.read_csv(pathFijo+'/ETBMOVCLA_DPTO_2018-2023.csv',delimiter=';',encoding='utf-8-sig').dropna(how='all') 
    df2_2Fijo=pd.read_csv(pathFijo+'/TIGOEMCALI_DPTO_2018-2023.csv',delimiter=';',encoding='utf-8-sig').dropna(how='all') 
    ...
    OpCiud2Fijo['Aggregate Date'] = OpCiud2Fijo['Aggregate Date'].replace(diction2Fijo) #Reemplazar fechas antiguas por nuevas
    OpCiud2Fijo['Aggregate Date'] =pd.to_datetime(OpCiud2Fijo['Aggregate Date'], format='%d-%b-%y').dt.floor('d') 
    ...
    geoJSON_states2 = list(gdf2.Location.values)
    denominations_json2 = []
    Id_json2 = []
...
####Tercera sección - Información por ciudades capitales
def Seccion3Fijo():
    Ciudades3Fijo = pd.read_csv(pathFijo+'/Mediana_Fijo_32Cities_Agr.Todos_Mensual-2018-2023(Dic).csv', delimiter=';', encoding='utf-8-sig')
    ...
    FeCorre3Fijo = pd.date_range('2018-01-01', periodo_tope, freq='MS').strftime("%d-%b-%y").tolist()  # lista de fechas en el periodo seleccionado
    diction3Fijo = dict(zip(FeAntig3Fijo, FeCorre3Fijo))
    Ciudades3Fijo['Aggregate Date'] = Ciudades3Fijo['Aggregate Date'].replace(diction3Fijo)  # Reemplazar fechas antiguas por nuevas
    Ciudades3Fijo['Aggregate Date'] = pd.to_datetime(Ciudades3Fijo['Aggregate Date'], format='%d-%b-%y')
...
####Cuarta sección - Información agregada por operadores
def Seccion4Fijo():
    Operadores4Fijo=pd.read_csv(pathFijo+'/Op-histcomp_month_2018-2023(Diciembre).csv',delimiter=';')
    ...
    Operadores4Fijo['Aggregate Date'] = Operadores4Fijo['Aggregate Date'].replace(diction4Fijo) #Reemplazar fechas antiguas por nuevas
    Operadores4Fijo['Aggregate Date'] = pd.to_datetime(Operadores4Fijo['Aggregate Date'], format='%d-%b-%y').dt.floor('d') 
...
####Quinta sección - Información departamental por operadores
Colombia2Fijo=pd.read_csv(pathFijo+'/Mediana_Fijo_Departamentos.Todos_Mensual_2020-2023.csv',delimiter=';')
...
Colombia2Fijo['Aggregate Date'] = Colombia2Fijo['Aggregate Date'].replace(dictionFijo) #Reemplazar fechas antiguas por nuevas
Colombia2Fijo['Aggregate Date'] =pd.to_datetime(Colombia2Fijo['Aggregate Date'],format='%d-%b-%y').dt.floor('d') 
...
#### Sexta sección - Ciudades principales por operador
def custom_sort_key(date_str):
    month, year = date_str.split()
    month_number = datetime.datetime.strptime(month, '%b').month
    return int(year) * 100 + month_number
...
TopCapDep=pd.read_csv(pathFijo+'/Mediana_Fijo_4City_6PRST&All_Mensual-2021-2023(Dic).csv',delimiter=';',encoding='utf-8')
TopCapDep.rename(columns={'Location':'DESC_MUNICIPIO'},inplace=True)
TopCapDep['DESC_MUNICIPIO']=TopCapDep['DESC_MUNICIPIO'].str.upper()
TopCapDep['Aggregate Date']=TopCapDep['Aggregate Date'].apply(standardize_date)
FeAntigFijoTCP= sorted(TopCapDep['Aggregate Date'].unique(),key=lambda x: datetime.datetime.strptime(x, '%b-%Y')) #Generar las fechas que tenían los datos
FeCorreFijoTCP=pd.date_range('2021-12-01',periodo_tope, 
              freq='MS').strftime("%d-%b-%y").tolist() #lista de fechas en el periodo seleccionado
dictionFijoTCP=dict(zip(FeAntigFijoTCP, FeCorreFijoTCP))
TopCapDep['Aggregate Date']=TopCapDep['Aggregate Date'].replace(dictionFijoTCP) #Reemplazar fechas antiguas por nuevas
TopCapDep['Aggregate Date'] =pd.to_datetime(TopCapDep['Aggregate Date'],format='%d-%b-%y').dt.floor('d') 
TopCapDep['mes']=pd.DatetimeIndex(TopCapDep['Aggregate Date']).month
TopCapDep['año']=pd.DatetimeIndex(TopCapDep['Aggregate Date']).year
TopCapDep['periodo']=TopCapDep['año'].astype('str')+'-'+TopCapDep['mes'].astype('str').str.zfill(2)
TopCapDep['DESC_MUNICIPIO']=TopCapDep['DESC_MUNICIPIO'].str.split(',',expand=True)[0]#Guardar sólo las ciudades


# Filtro calidad Ookla
TopCapDepcond = TopCapDep.groupby(['año', 'DESC_MUNICIPIO','Provider'])['User Count'].agg(['count', lambda x: (x > 200).sum()])
TopCapDepcond.rename(columns={'count':'meses_obs','<lambda_0>':'Cumple'},inplace=True)
TopCapDepcond['Porc_cumplimiento'] = (TopCapDepcond['Cumple'] / TopCapDepcond['meses_obs']) 
filtroTopCapDepcond = TopCapDepcond[TopCapDepcond['Porc_cumplimiento'] < 0.75]
annos_excluirTopCapDepcond = filtroTopCapDepcond.index.get_level_values('año').tolist()
muni_excluirTopCapDepcond = filtroTopCapDepcond.index.get_level_values('DESC_MUNICIPIO').tolist()
provider_excluirTopCapDepcond = filtroTopCapDepcond.index.get_level_values('Provider').tolist()
filtro_completoTopCapDep = None
for year, location, provider in zip(annos_excluirTopCapDepcond, muni_excluirTopCapDepcond,provider_excluirTopCapDepcond):
    condition_TopCapDep = (TopCapDep['año'] == year) & (TopCapDep['DESC_MUNICIPIO'] == location) & (TopCapDep['Provider'] == provider)
    if filtro_completoTopCapDep is None:
        filtro_completoTopCapDep = condition_TopCapDep
    else:
        filtro_completoTopCapDep = filtro_completoTopCapDep | condition_TopCapDep
TopCapDep = TopCapDep[~filtro_completoTopCapDep]
TopCapDep = TopCapDep[TopCapDep['periodo']>='2021-12']
###########################################################################################################################################################

CapDep=['LETICIA','MEDELLÍN','ARAUCA','BARRANQUILLA','BOGOTÁ','CARTAGENA','TUNJA','MANIZALES','FLORENCIA','YOPAL','POPAYÁN',
...
if select_servicio == 'Internet fijo':
    ...
    if select_indicador== 'Velocidad de descarga':
        ...
        if dimension_Vel_descarga_Fijo == 'Histórico Colombia':
            ...
            with tab1:
                ...
                zeroline=True,linecolor = "#000000",zerolinewidth=2,showticklabels=True) 
                fig1Fijo.update_traces(textfont_size=22)
                ...
                title={
                'text': "<b>Velocidad mensual de descarga de Internet fijo en <br>Colombia (2018-2023) (en Mbps) </b>",
                'y':0.9,
                'x':0.5,
                'xanchor': 'center',
                'yanchor': 'top'})  
        ...
        if dimension_Vel_descarga_Fijo == 'Ciudades':  
            ...
            with tab1:
                ...
                fig2Fijo.update_xaxes(tickangle=-90, tickfont=dict(family='Tahoma', color='black', size=22),title_font=dict(family="Tahoma"),title_text=None,ticks="outside",tickwidth=1, tickcolor='black', ticklen=5,
                zeroline=True,linecolor = "#000000",zerolinewidth=2,showticklabels=True)
                fig2Fijo.update_yaxes(range=[0,max(DepJoinA3Fijo['2023'].values.tolist())+5],tickfont=dict(family='Tahoma', color='black', size=22),title_font=dict(family="Tahoma"),titlefont_size=22, title_text="Velocidad descarga (Mbps)",ticks="outside", tickwidth=1, tickcolor='black', ticklen=5,
                zeroline=True,linecolor = "#000000",zerolinewidth=2,showticklabels=True) 
            ...
            with tab2:
                ...
                with col2:
                    Mes_opFijoDer = st.selectbox('Escoja el mes de 2023 para el panel de la derecha',['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'],index=11)           
                ...
                fig4Fijo.update_xaxes(tickangle=0,range=[0,max(DicDer['Download Speed Mbps'].values.tolist())+10],tickfont=dict(family='Tahoma', color='black', size=22),title_font=dict(family="Tahoma"),ticks="outside",tickwidth=1, tickcolor='black', ticklen=5,
                zeroline=True,linecolor = "#000000",zerolinewidth=2,showticklabels=True)
                ...
                fig4Fijo.update_xaxes(showspikes=True,title_text='Velocidad de descarga (Mbps)',titlefont_size=22)
            ...
            with tab3:
                ...
                for op in TopCapDep_cap['Provider'].unique():
                    ...
                    fig_muniOp.add_trace(go.Scatter(x=df_muniOp['periodo'],y=df_muniOp['Download Speed Mbps'],mode='lines+markers',name=op,marker_color=color_op
                    ))
                    ...
                    zeroline=True,linecolor = "#000000",zerolinewidth=2,showticklabels=True)
                    fig_muniOp.update_yaxes(tickfont=dict(family='Tahoma', color='black', size=22),title_font=dict(family="Tahoma"),titlefont_size=22, title_text='Velocidad (Mbps)',ticks="outside", tickwidth=1, tickcolor='black', ticklen=5,
                    zeroline=True,linecolor = "#000000",zerolinewidth=2,showticklabels=True) 
        ...
        if dimension_Vel_descarga_Fijo == 'Operadores': 
            ...
            with tab1:
                ...
                fig3Fijo.update_xaxes(tickangle=0, tickfont=dict(family='Tahoma', color='black', size=22),title_text=None,ticks="outside", tickformat="%m<br>20%y",tickwidth=1, tickcolor='black', ticklen=5,
                zeroline=True,linecolor = "#000000",zerolinewidth=2,showticklabels=True)
                fig3Fijo.update_yaxes(tickfont=dict(family='Tahoma', color='black', size=22),title_font=dict(family="Tahoma"),titlefont_size=22, title_text='Velocidad descarga (Mbps)',ticks="outside", tickwidth=1, tickcolor='black', ticklen=5,
                zeroline=True,linecolor = "#000000",zerolinewidth=2,showticklabels=True) 
                fig3Fijo.update_traces(textfont_size=22)
                fig3Fijo.update_layout(height=500,legend_title=None,font=dict(size=22))
                ...
                title={
                'text': "<b>Velocidad mensual de descarga de Internet fijo<br>por proveedor (2018-2023) (en Mbps)</b>",
                'y':0.85,
                'x':0.5,
                'xanchor': 'center',
                'yanchor': 'top'})  
    ...
    #            with col2b:
            ...
            with tab3:
                ...
                fig5Fijo.update_xaxes(range=[0,170],tickangle=0,tickfont=dict(family='Tahoma', color='black', size=22),title_font=dict(family="Tahoma"),ticks="outside",tickwidth=1, tickcolor='black', ticklen=5,
                zeroline=True,linecolor = "#000000",zerolinewidth=2,showticklabels=True)
                fig5Fijo.update_yaxes(range=[0,170],tickfont=dict(family='Tahoma', color='black', size=22),title_font=dict(family="Tahoma"),titlefont_size=22,ticks="outside", tickwidth=1, tickcolor='black', ticklen=5,
                zeroline=True,linecolor = "#000000",zerolinewidth=2,showticklabels=True) 
                fig5Fijo.update_traces(textfont_size=22)
                ...
                fig5Fijo.update_layout(font_color="Black",title_font_family="Tahoma",title_font_color="Black",titlefont_size=22,
                title={
    ...
    if select_indicador== 'Velocidad de carga':
        ...
        if dimension_Vel_carga_Fijo == 'Histórico Colombia':
            ...
            with tab1:
                ...
                zeroline=True,linecolor = "#000000",zerolinewidth=2,showticklabels=True)
                fig6Fijo.update_yaxes(tickfont=dict(family='Tahoma', color='black', size=22),title_font=dict(family="Tahoma"),titlefont_size=22, title_text='Velocidad carga<br>(Mbps)',ticks="outside", tickwidth=1, tickcolor='black', ticklen=5,
                zeroline=True,linecolor = "#000000",zerolinewidth=2,showticklabels=True) 
                fig6Fijo.update_traces(textfont_size=22)
                ...
                title={
                'text': "<b>Velocidad mensual de carga de Internet fijo en Colombia (2018-2023) (en Mbps)</b>",
                'y':0.9,
                'x':0.5,
                'xanchor': 'center',
                'yanchor': 'top'})  
        ...
        if dimension_Vel_carga_Fijo == 'Ciudades':    
            ...
            with tab1:
                ...
                fig7Fijo.update_xaxes(tickangle=-90, tickfont=dict(family='Tahoma', color='black', size=22),title_text=None,ticks="outside",tickwidth=1, tickcolor='black', ticklen=5,
                zeroline=True,linecolor = "#000000",zerolinewidth=2,showticklabels=True)
                fig7Fijo.update_yaxes(range=[0,max(DepJoinB3['2023'].values.tolist())+5],tickfont=dict(family='Tahoma', color='black', size=22),title_font=dict(family="Tahoma"),titlefont_size=22, title_text="Velocidad carga promedio (Mbps)",ticks="outside", tickwidth=1, tickcolor='black', ticklen=5,
                zeroline=True,linecolor = "#000000",zerolinewidth=2,showticklabels=True) 
                ...
                fig7Fijo.update_layout(font_color="Black",title_font_family="Tahoma",title_font_color="Black",titlefont_size=22,font=dict(size=22),
                title={
                'text': "<b>Velocidad anual de carga de Internet fijo por ciudad<br> (2020-2023)</b>",
                'y':0.95,
                'x':0.5,
                'xanchor': 'center',
                'yanchor': 'top'})  
        ...
        if dimension_Vel_carga_Fijo == 'Operadores':   
            ...
            with tab1:            
                ...
                fig8Fijo.update_xaxes(tickangle=0, tickfont=dict(family='Tahoma', color='black', size=22),title_text=None,ticks="outside", tickformat="%m<br>20%y",tickwidth=1, tickcolor='black', ticklen=5,
                zeroline=True,linecolor = "#000000",zerolinewidth=2,showticklabels=True)
                fig8Fijo.update_yaxes(tickfont=dict(family='Tahoma', color='black', size=22),title_font=dict(family="Tahoma"),titlefont_size=22, title_text='Velocidad de carga (Mbps)',ticks="outside", tickwidth=1, tickcolor='black', ticklen=5,
                zeroline=True,linecolor = "#000000",zerolinewidth=2,showticklabels=True) 
                fig8Fijo.update_traces(textfont_size=14)
                fig8Fijo.update_layout(height=500,legend_title=None,font=dict(size=22))
                ...
                fig8Fijo.update_layout(font_color="Black",title_font_family="Tahoma",title_font_color="Black",titlefont_size=22,
                title={
                'text': "<b>Velocidad mensual de carga de Internet fijo<br>por proveedor (2018-2023) (en Mbps)</b>",
                'y':0.85,
                'x':0.5,
                'xanchor': 'center',
                'yanchor': 'top'})  
    ...
    if select_indicador== 'Latencia':            
        ...
        if dimension_Latencia_Fijo == 'Histórico Colombia':    
            ...
            with tab1:
                ...
                zeroline=True,linecolor = "#000000",zerolinewidth=2,showticklabels=True) 
                fig9Fijo.update_traces(textfont_size=22)
                ...
                title={
                'text': "<b>Latencia mensual de Internet fijo en Colombia<br>(2018-2023)</b>",
                'y':0.9,
                'x':0.5,
                'xanchor': 'center',
                'yanchor': 'top'})  
        ...
        if dimension_Latencia_Fijo == 'Ciudades':    
            ...
            with tab1:   
                ...
                fig10Fijo.update_xaxes(tickangle=-90, tickfont=dict(family='Tahoma', color='black', size=22),title_text=None,ticks="outside",tickwidth=1, tickcolor='black', ticklen=5,
                zeroline=True,linecolor = "#000000",zerolinewidth=2,showticklabels=True)
                fig10Fijo.update_yaxes(range=[0,100],tickfont=dict(family='Tahoma', color='black', size=22),title_font=dict(family="Tahoma"),titlefont_size=22, title_text="Latencia (ms)",ticks="outside", tickwidth=1, tickcolor='black', ticklen=5,
                zeroline=True,linecolor = "#000000",zerolinewidth=2,showticklabels=True) 
                ...
                fig10Fijo.update_layout(font_color="Black",title_font_family="Tahoma",title_font_color="Black",titlefont_size=22,font=dict(size=22),
                title={
                'text': "<b>Latencia anual de Internet fijo por ciudad en ms<br> (2019-2023) </b>",
                'y':0.9,
                'x':0.5,
                'xanchor': 'center',
                'yanchor': 'top'})  
            ...
            with tab2:    
                ...
                for op in TopCapDep_cap['Provider'].unique():
                    ...
                    fig_muniOp.update_xaxes(tickangle=0, tickfont=dict(family='Tahoma', color='black', size=22),title_text=None,ticks="outside", tickformat="%m<br>20%y",tickwidth=1, tickcolor='black', ticklen=5,
                    zeroline=True,linecolor = "#000000",zerolinewidth=2,showticklabels=True)
                    fig_muniOp.update_yaxes(tickfont=dict(family='Tahoma', color='black', size=22),title_font=dict(family="Tahoma"),titlefont_size=22, title_text='Latencia (ms)',ticks="outside", tickwidth=1, tickcolor='black', ticklen=5,
                    zeroline=True,linecolor = "#000000",zerolinewidth=2,showticklabels=True) 
                    fig_muniOp.update_traces(textfont_size=22)
                    fig_muniOp.update_layout(height=500,legend_title=None,font=dict(size=22))
                    fig_muniOp.update_layout(legend=dict(orientation="h",y=1.1,xanchor="center",x=0.5),showlegend=True)
        ...
        if dimension_Latencia_Fijo == 'Operadores':  
            ...
            with tab1:                        
                ...
                fig11Fijo.update_xaxes(tickangle=0, tickfont=dict(family='Tahoma', color='black', size=22),title_text=None,ticks="outside", tickformat="%m<br>20%y",tickwidth=1, tickcolor='black', ticklen=5,
                zeroline=True,linecolor = "#000000",zerolinewidth=2,showticklabels=True)
                ...
                fig11Fijo.update_layout(font_color="Black",title_font_family="Tahoma",title_font_color="Black",titlefont_size=22,
                title={
                'text': "<b>Latencia mensual de Internet fijo en Colombia (2018-2023) (en ms)</b>",
                'y':0.85,
                'x':0.5,
                'xanchor': 'center',
                'yanchor': 'top'})  
...
fijos_int=pd.read_csv(pathFijo+'Ookla_Internacional-Fijos_032024.csv',delimiter=';')
fijos_int.rename(columns={'Country':'País'},inplace=True)
fijos_int=fijos_int[fijos_int['Date']=='2023-12-01']
...
if select_servicio == 'Comparación internacional':
    ...
    fig1Int.update_xaxes(tickangle=0, tickfont=dict(family='Tahoma', color='black', size=22),titlefont_family='tahoma',titlefont_size=22,title_text='Velocidad de descarga (Mbps)',ticks="outside",tickwidth=1, tickcolor='black', ticklen=5,
    zeroline=True,linecolor = "#000000",zerolinewidth=2,showticklabels=True)
    fig1Int.update_yaxes(tickfont=dict(family='Tahoma', color='black', size=22),titlefont_family='tahoma',titlefont_size=22, title_text='Velocidad de carga (Mbps)',ticks="outside", tickwidth=1, tickcolor='black', ticklen=5,
    zeroline=True,linecolor = "#000000",zerolinewidth=2,showticklabels=True) 
